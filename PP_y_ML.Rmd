...
#### 3.2.2 Versión de www.khstats.com/blog/corr-plots
...
indice_entrenamiento <- sample(1:nrow(df), 0.7 * nrow(df))
datos_entrenamiento <- df[indice_entrenamiento, ]
datos_prueba <- df[-indice_entrenamiento, ]

# Entrenamiento del modelo de regresión lineal
modelo <- lm(Remuneracion_media ~ clae2 + Cantidad_trabajadores + Cantidad_trabajadores_Hombres + Cantidad_trabajadores_Mujeres + Poblacion_radio + Viviendas_radio, data = datos_entrenamiento)
...
```{r bloque080, eval=ML, include=ML}
...
xyplot(Remuneracion_media_Hombres ~ Remuneracion_media_Mujeres | provincia_id,
...
```{r bloque082, eval=ML, include=MLL}
set.seed(001)
...
```{r bloque083, eval=ML, include=MLL}
...
```{r, eval=ML, include=MLL}
utils::View(df_x_depto)
```

```{r, eval=ML, include=ML}
  ggplot(data = df_a_letra, aes(x = Remuneracion_media, y = Remuneracion_media_Hombres)) +
  geom_point() +
...
```{r, eval=ML, include=ML}
modelo <- lm(formula = Remuneracion_media_Hombres ~ Remuneracion_media_Mujeres, data = df_x_depto)
```
